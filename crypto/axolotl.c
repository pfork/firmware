...
static int tripledh(uint8_t *mk, u8 send[POLY_BYTES], const Axolotl_prekey_private *ctx, const Axolotl_PreKey *prekey, int isAlice) {
  ...
  */
  ...
  ptr+=crypto_scalarmult_curve25519_BYTES;
  if(send!=NULL) newhope_sharedb(ptr, send, prekey->newhope);
...
// resp needs to be non-null for the first handshaker, the initiator
int axolotl_handshake(Axolotl_ctx* ctx, Axolotl_PreKey *resp, const Axolotl_PreKey *prekey, Axolotl_prekey_private *private) {
  ...
  */
  ...
  // check sig on prekey
  if(xed25519_verify(prekey->sig, prekey->identitykey,
                     ((uint8_t*) prekey)+crypto_scalarmult_curve25519_BYTES,
  ...
  //crypto_scalarmult_curve25519_base(pk, private->dhis); // derive pubkey
  ...
  ctx->isAlice=memcmp(pk, prekey->identitykey, crypto_scalarmult_curve25519_BYTES) <=0;

  if(resp!=NULL) { // init private
    // perform triple DH to derive master key
    if(tripledh(mk, resp->newhope, private, prekey, ctx->isAlice)!=0) return 1;
  } else {
    if(tripledh(mk, NULL, private, prekey, ctx->isAlice)!=0) return 1;
  }

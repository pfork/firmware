...
static void USB_OTG_EnableCommonInt(USB_OTG_CORE_HANDLE *pdev) {
  ...
  WRITE_REG32( &pdev->regs.GREGS->GINTSTS, 0xFFFFFFFF);
  /* Enable the interrupts in the INTMSK */
  int_mask.b.wkupintr = 1;
  int_mask.b.usbsuspend = 1;

  WRITE_REG32( &pdev->regs.GREGS->GINTMSK, int_mask.d32);
}
...
static USB_OTG_STS USB_OTG_CoreReset(USB_OTG_CORE_HANDLE *pdev) {
  ...
  volatile USB_OTG_GRSTCTL_TypeDef  greset;
  ...
  do {
    uDelay(3);
    greset.d32 = MMIO32(&pdev->regs.GREGS->GRSTCTL);
    if (++count > 200000) {
      return USB_OTG_OK;
    }
  ...
  count = 0;
  greset.b.csftrst = 1;
  WRITE_REG32(&pdev->regs.GREGS->GRSTCTL, greset.d32 );
  do {
    greset.d32 = MMIO32(&pdev->regs.GREGS->GRSTCTL);
    if (++count > 200000) {
      break;
...
* @retval USB_OTG_STS : status
*/
USB_OTG_STS USB_OTG_WritePacket(USB_OTG_CORE_HANDLE *pdev, unsigned char *src, unsigned char ch_ep_num, unsigned short len) {
  USB_OTG_STS status = USB_OTG_OK;
  if (pdev->cfg.dma_enable == 0) {
    unsigned int count32b= 0, i= 0;
    volatile unsigned int *fifo;

    count32b =  (len + 3) / 4;
    fifo = pdev->regs.DFIFO[ch_ep_num];
    for (i = 0; i < count32b; i++, src+=4) {
      WRITE_REG32( fifo, *((__packed unsigned int *)src) );
    }
  }
  return status;
}
...
* @retval None
*/
void *USB_OTG_ReadPacket(USB_OTG_CORE_HANDLE *pdev, unsigned char *dest, unsigned short len) {
  unsigned int i=0;
  ...
  for ( i = 0; i < count32b; i++, dest += 4 ) {
    *(__packed unsigned int *)dest = MMIO32(fifo);
 }
  return ((void *)dest);
}
...
USB_OTG_STS USB_OTG_SelectCore(USB_OTG_CORE_HANDLE *pdev) {
  ...
  pdev->regs.HPRT0 = (unsigned int *) (baseAddress + USB_OTG_HOST_PORT_REGS_OFFSET);
  ...
  for (i = 0; i < pdev->cfg.host_channels; i++) {
    pdev->regs.DFIFO[i] = ((unsigned int *)(baseAddress + USB_OTG_DATA_FIFO_OFFSET +
                                           (i * USB_OTG_DATA_FIFO_SIZE)));
  }
  pdev->regs.PCGCCTL = (unsigned int *)(baseAddress + USB_OTG_PCGCCTL_OFFSET);
...
*         Initializes the USB_OTG controller registers and prepares the
*         core device mode
* @param  pdev : Selected device
* @retval USB_OTG_STS : status
*/
USB_OTG_STS USB_OTG_CoreInit(USB_OTG_CORE_HANDLE *pdev) {
  ...
  usbcfg.d32 = MMIO32(&pdev->regs.GREGS->GUSBCFG);;
  usbcfg.b.physel  = 1; /* FS Interface */
  WRITE_REG32 (&pdev->regs.GREGS->GUSBCFG, usbcfg.d32);
  ...
  WRITE_REG32 (&pdev->regs.GREGS->GCCFG, gccfg.d32);
  mDelay(20);
  ...
  if(pdev->cfg.phy_itface == USB_OTG_I2C_PHY) {
    ...
    i2cctl.d32 = MMIO32(&pdev->regs.GREGS->GI2CCTL);
    i2cctl.b.i2cdevaddr = 1;
    i2cctl.b.i2cen = 0;
    i2cctl.b.dat_se0 = 1;
    i2cctl.b.addr = 0x2D;
    WRITE_REG32 (&pdev->regs.GREGS->GI2CCTL, i2cctl.d32);
...
USB_OTG_STS USB_OTG_EnableGlobalInt(USB_OTG_CORE_HANDLE *pdev) {
  ...
  MODIFY_REG32(&pdev->regs.GREGS->GAHBCFG, 0, ahbcfg.d32);
  return status;
}
...
USB_OTG_STS USB_OTG_DisableGlobalInt(USB_OTG_CORE_HANDLE *pdev) {
  ...
  ahbcfg.d32 = 0;
  ahbcfg.b.glblintrmsk = 1; /* Enable interrupts */
  MODIFY_REG32(&pdev->regs.GREGS->GAHBCFG, ahbcfg.d32, 0);
  return status;
}
...
unsigned int USB_OTG_GetMode(USB_OTG_CORE_HANDLE *pdev) {
  return (MMIO32(&pdev->regs.GREGS->GINTSTS ) & 0x1);
}
...
unsigned int USB_OTG_ReadCoreItr(USB_OTG_CORE_HANDLE *pdev) {
  unsigned int v = 0;
  v = MMIO32(&pdev->regs.GREGS->GINTSTS);
  v &= MMIO32(&pdev->regs.GREGS->GINTMSK);
  return v;
}
...
unsigned int USB_OTG_ReadOtgItr (USB_OTG_CORE_HANDLE *pdev) {
  return (MMIO32 (&pdev->regs.GREGS->GOTGINT));
}
...
USB_OTG_STS USB_OTG_CoreInitDev (USB_OTG_CORE_HANDLE *pdev) {
  ...
  /* set Rx FIFO size */
  WRITE_REG32(&pdev->regs.GREGS->GRXFSIZ, RX_FIFO_FS_SIZE);

  /* EP0 TX*/
  nptxfifosize.b.depth     = TX0_FIFO_FS_SIZE;
  nptxfifosize.b.startaddr = RX_FIFO_FS_SIZE;
  WRITE_REG32( &pdev->regs.GREGS->DIEPTXF0_HNPTXFSIZ, nptxfifosize.d32 );


  /* EP1 TX*/
  txfifosize.b.startaddr = nptxfifosize.b.startaddr + nptxfifosize.b.depth;
  txfifosize.b.depth = TX1_FIFO_FS_SIZE;
  WRITE_REG32( &pdev->regs.GREGS->DIEPTXF[0], txfifosize.d32 );


  /* EP2 TX*/
  txfifosize.b.startaddr += txfifosize.b.depth;
  txfifosize.b.depth = TX2_FIFO_FS_SIZE;
  WRITE_REG32( &pdev->regs.GREGS->DIEPTXF[1], txfifosize.d32 );


  /* EP3 TX*/
  txfifosize.b.startaddr += txfifosize.b.depth;
  txfifosize.b.depth = TX3_FIFO_FS_SIZE;
  WRITE_REG32( &pdev->regs.GREGS->DIEPTXF[2], txfifosize.d32 );

  /* Flush the FIFOs */
  USB_OTG_FlushTxFifo(pdev, 0x10); /* all Tx FIFOs */
  USB_OTG_FlushRxFifo(pdev);
  /* Clear all pending Device Interrupts */
  WRITE_REG32( &pdev->regs.DREGS->DIEPMSK, 0 );
  WRITE_REG32( &pdev->regs.DREGS->DOEPMSK, 0 );
  WRITE_REG32( &pdev->regs.DREGS->DAINT, 0xFFFFFFFF );
  WRITE_REG32( &pdev->regs.DREGS->DAINTMSK, 0 );

  for (i = 0; i < pdev->cfg.dev_endpoints; i++) {
    depctl.d32 = MMIO32(&pdev->regs.INEP_REGS[i]->DIEPCTL);
    if (depctl.b.epena) {
      depctl.d32 = 0;
      depctl.b.epdis = 1;
      depctl.b.snak = 1;
    ...
    WRITE_REG32( &pdev->regs.INEP_REGS[i]->DIEPCTL, depctl.d32);
    WRITE_REG32( &pdev->regs.INEP_REGS[i]->DIEPTSIZ, 0);
    WRITE_REG32( &pdev->regs.INEP_REGS[i]->DIEPINT, 0xFF);
  }
  for (i = 0; i <  pdev->cfg.dev_endpoints; i++) {
    USB_OTG_DEPCTL_TypeDef  depctl;
    depctl.d32 = MMIO32(&pdev->regs.OUTEP_REGS[i]->DOEPCTL);
    if (depctl.b.epena) {
      depctl.d32 = 0;
      depctl.b.epdis = 1;
      depctl.b.snak = 1;
    ...
    WRITE_REG32( &pdev->regs.OUTEP_REGS[i]->DOEPCTL, depctl.d32);
    WRITE_REG32( &pdev->regs.OUTEP_REGS[i]->DOEPTSIZ, 0);
    WRITE_REG32( &pdev->regs.OUTEP_REGS[i]->DOEPINT, 0xFF);
  }
  msk.d32 = 0;
  msk.b.txfifoundrn = 1;
  MODIFY_REG32(&pdev->regs.DREGS->DIEPMSK, msk.d32, msk.d32);
  ...
  if (pdev->cfg.dma_enable == 1) {
    ...
    WRITE_REG32(&pdev->regs.DREGS->DTHRCTL, dthrctl.d32);
  }
...
USB_OTG_STS USB_OTG_EnableDevInt(USB_OTG_CORE_HANDLE *pdev) {
  ...
  WRITE_REG32( &pdev->regs.GREGS->GINTMSK, 0);
  /* Clear any pending interrupts */
  WRITE_REG32( &pdev->regs.GREGS->GINTSTS, 0xFFFFFFFF);
  ...
  intmsk.b.otgintr    = 1;
#endif
  MODIFY_REG32( &pdev->regs.GREGS->GINTMSK, intmsk.d32, intmsk.d32);
  return status;
}
...
USB_OTG_STS  USB_OTG_EP0Activate(USB_OTG_CORE_HANDLE *pdev) {
  ...
  diepctl.d32 = MMIO32(&pdev->regs.INEP_REGS[0]->DIEPCTL);
  ...
  WRITE_REG32(&pdev->regs.INEP_REGS[0]->DIEPCTL, diepctl.d32);
  dctl.b.cgnpinnak = 1;
  MODIFY_REG32(&pdev->regs.DREGS->DCTL, dctl.d32, dctl.d32);
  return status;
}
...
USB_OTG_STS USB_OTG_EPActivate(USB_OTG_CORE_HANDLE *pdev, USB_OTG_EP *ep) {
  ...
  * register. */
  ...
  MODIFY_REG32(&pdev->regs.DREGS->DAINTMSK, 0, daintmsk.d32);
  return status;
}
...
USB_OTG_STS USB_OTG_EPDeactivate(USB_OTG_CORE_HANDLE *pdev, USB_OTG_EP *ep) {
  ...
  MODIFY_REG32(&pdev->regs.DREGS->DAINTMSK, daintmsk.d32, 0);
  return status;
}
...
USB_OTG_STS USB_OTG_EPStartXfer(USB_OTG_CORE_HANDLE *pdev, USB_OTG_EP *ep) {
  ...
  /* IN endpoint */
  if (ep->is_in == 1) {
    in_regs = pdev->regs.INEP_REGS[ep->num];
    depctl.d32  = MMIO32(&(in_regs->DIEPCTL));
    deptsiz.d32 = MMIO32(&(in_regs->DIEPTSIZ));
    /* Zero Length Packet? */
    if (ep->xfer_len == 0) {
    ...
    if (pdev->cfg.dma_enable == 1) {
      WRITE_REG32(&in_regs->DIEPDMA, ep->dma_addr);
    } else {
      if (ep->type != EP_TYPE_ISOC) {
        /* Enable the Tx FIFO Empty Interrupt for this EP */
        if (ep->xfer_len > 0) {
          fifoemptymsk = 1 << ep->num;
          MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, 0, fifoemptymsk);
        }
      }
    }
    ...
    /* OUT endpoint */
    out_regs = pdev->regs.OUTEP_REGS[ep->num];
    depctl.d32  = MMIO32(&(out_regs->DOEPCTL));
    deptsiz.d32 = MMIO32(&(out_regs->DOEPTSIZ));
    ...
    */
    ...
    WRITE_REG32(&out_regs->DOEPTSIZ, deptsiz.d32);

    if (pdev->cfg.dma_enable == 1) {
      WRITE_REG32(&out_regs->DOEPDMA, ep->dma_addr);
    }
    ...
    depctl.b.cnak = 1;
    depctl.b.epena = 1;
    WRITE_REG32(&out_regs->DOEPCTL, depctl.d32);
  }
  return status;
}
...
USB_OTG_STS USB_OTG_EP0StartXfer(USB_OTG_CORE_HANDLE *pdev, USB_OTG_EP *ep) {
  ...
  if (ep->is_in == 1) {
    ...
    if (pdev->cfg.dma_enable == 1) {
      WRITE_REG32(&pdev->regs.INEP_REGS[ep->num]->DIEPDMA, ep->dma_addr);
    }
    ...
    /* OUT endpoint */
    out_regs = pdev->regs.OUTEP_REGS[0];
    depctl.d32  = MMIO32(&out_regs->DOEPCTL);
    deptsiz.d32 = MMIO32(&out_regs->DOEPTSIZ);
    /* Program the transfer size and packet count as follows:
...
unsigned int USB_OTG_ReadDevAllOutEp_itr(USB_OTG_CORE_HANDLE *pdev) {
  unsigned int v;
  v  = MMIO32(&pdev->regs.DREGS->DAINT);
  v &= MMIO32(&pdev->regs.DREGS->DAINTMSK);
...
unsigned int USB_OTG_ReadDevOutEP_itr(USB_OTG_CORE_HANDLE *pdev, unsigned char epnum) {
  unsigned int v;
  v  = MMIO32(&pdev->regs.OUTEP_REGS[epnum]->DOEPINT);
  v &= MMIO32(&pdev->regs.DREGS->DOEPMSK);
  return v;
}
...
void USB_OTG_EP0_OutStart(USB_OTG_CORE_HANDLE *pdev) {
  ...
  WRITE_REG32( &pdev->regs.OUTEP_REGS[0]->DOEPTSIZ, doeptsize0.d32 );

  if (pdev->cfg.dma_enable == 1) {
    USB_OTG_DEPCTL_TypeDef  doepctl;
    doepctl.d32 = 0;
    WRITE_REG32( &pdev->regs.OUTEP_REGS[0]->DOEPDMA,
                        (unsigned int)&pdev->dev.setup_packet);

    /* EP enable */
    doepctl.d32 = MMIO32(&pdev->regs.OUTEP_REGS[0]->DOEPCTL);
    doepctl.b.epena = 1;
    doepctl.d32 = 0x80008000;
    WRITE_REG32( &pdev->regs.OUTEP_REGS[0]->DOEPCTL, doepctl.d32);
  }
}

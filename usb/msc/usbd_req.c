...
__ALIGN_BEGIN unsigned int USBD_ep_status __ALIGN_END  = 0;
__ALIGN_BEGIN unsigned int  USBD_default_cfg __ALIGN_END  = 0;
__ALIGN_BEGIN unsigned int  USBD_cfg_status __ALIGN_END  = 0;
__ALIGN_BEGIN unsigned char USBD_StrDesc[USB_MAX_STR_DESC_SIZ] __ALIGN_END ;
...
static unsigned char USBD_GetLen(unsigned char *buf);
...
static void USBD_GetDescriptor(USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ *req) {
  ...
  case USB_DESC_TYPE_CONFIGURATION:
      pbuf   = (unsigned char *)pdev->dev.class_cb->GetConfigDescriptor(pdev->cfg.speed, &len);
...
static void USBD_GetConfig(USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ *req) {
  if (req->wLength != 1) {
    ...
    switch (pdev->dev.device_status ) {
    case USB_OTG_ADDRESSED:
      USBD_CtlSendData (pdev, (unsigned char *)&USBD_default_cfg, 1);
      break;
    case USB_OTG_CONFIGURED:
...
static void USBD_GetStatus(USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ *req) {
  ...
  case USB_OTG_CONFIGURED:
    ...
    USBD_CtlSendData (pdev, (unsigned char *)&USBD_cfg_status, 1);
    break;
...
static void USBD_SetFeature(USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ *req) {
  ...
  if (req->wValue == USB_FEATURE_REMOTE_WAKEUP) {
    ...
    USBD_CtlSendStatus(pdev);
             ...
             ((req->wIndex & 0xFF) == 0)) {
    dctl.d32 = MMIO32(&pdev->regs.DREGS->DCTL);
    ...
    WRITE_REG32(&pdev->regs.DREGS->DCTL, dctl.d32);
    USBD_CtlSendStatus(pdev);
  }
...
void USBD_ParseSetupRequest( USB_OTG_CORE_HANDLE  *pdev, USB_SETUP_REQ *req) {
  req->bmRequest     = *(unsigned char *)  (pdev->dev.setup_packet);
  req->bRequest      = *(unsigned char *)  (pdev->dev.setup_packet +  1);

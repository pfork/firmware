...
int crypto_sign(unsigned char *sm,unsigned long long *smlen, const unsigned char *m,unsigned long long mlen, const unsigned char *sk)
{
  ...
  // create leafidx deterministically
  {
    ...
    //blake512((unsigned char*)rnd, scratch, SK_RAND_SEED_BYTES + mlen); //XXX: Why Blake 512?
    crypto_generichash((unsigned char*) rnd,64,scratch,SK_RAND_SEED_BYTES+mlen,NULL,0);

    // wipe sk
    sodium_memzero(scratch,SK_RAND_SEED_BYTES);
...
#error "Implemented for MESSAGE_HASH_SEED_BYTES == 32!"
  ...
  *smlen += horst_sigbytes;
  ...
  sodium_memzero(tsk, CRYPTO_SECRETKEYBYTES);
